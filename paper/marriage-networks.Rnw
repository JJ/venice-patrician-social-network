...
ggplot(marriages.summary, aes(x=intra.marriages,y=diff))+geom_point()
@

We will want to check how different ways of integrating intra-links affect actor-level measurements; as a benchmark we show the ranking for different relevant social network centrality measures: Degree, betweenness centrality, eigenvalue centrality and pagerank, computed on the original dataset (Tables \ref{tab:top:degree}, \ref{tab:top:betweenness}, \ref{tab:top:EV} and \ref{tab:top:pr}). Of these measurements, degree, which is simply the number of edges that link a node with others, is included mainly to have a indication of how many new links have been added to every node, and how these affect the top families. Betweenness centrality \cite{betweenness} is an important measure of how a family is able to intermediate between others; it is a good first approximation to the power, reputation or influence a family has. 
