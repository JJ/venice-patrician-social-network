...
<<r dup, echo=F >>=
marriages.dup <- marriages.sn
for ( n in intra.marriages ) {
  new.name <- paste0(n,"'")
  marriages.dup <- marriages.dup + vertex(new.name,color="yellow") + edge(n,new.name,weight=marriages.table[marriages.table$value == n,]$n.y)
  neighborhood <- neighbors(marriages.dup,V(marriages.dup)[n])
}
@
