...
We will want to check how different ways of integrating intra-links affect actor-level measurements; this is why we show the ranking for different relevant social network centrality measures in Tables \ref{tab:top:degree}, \ref{tab:top:betweenness}, \ref{tab:top:EV} and \ref{tab:top:pr}. However, as indicated in the introduction, this leaves out all data related to intra-family marriages, the top 10 of which is shown in Table \ref{tab:intra}. 

<<r plot.intra, echo=F, message=F,warning=F>>=
library(ggplot2)
ggplot(marriages.table,aes(x=n.x, y = n.y))+geom_point()
@
