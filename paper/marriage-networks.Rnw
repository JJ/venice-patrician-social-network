...
In all cases, centrality measures will be using the \emph{undirected}, \emph{non-normalized} version. In the same vein, we do not normalize centrality measures by network size since we are more interested in how our method changes ranking of the nodes than in comparing the graphs before and after the introduction of intra-family links.
