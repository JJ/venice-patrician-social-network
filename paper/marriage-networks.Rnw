...
All three methods take into account the importance of the intra-family marriages, which is reflected in the last column, "link importance", indicating that it reflects not only the existence of the links, but also the importance of these links. All methods add new edges to the network, which can be weighted, if needed. This does not differentiate them, but it is still a sanity check that, whichever method is chosen, can incorporate weights so that community detection algorithms, or weighted versions of the centrality measures can be applied to them.
