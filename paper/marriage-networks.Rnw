...
<<r marriage.data, echo=FALSE,cache=T,warning=FALSE,message=FALSE>>=
library(igraph)
library(knitr)
library(stringr)
...
marriages <- marriages[ marriages$wife_familyname_std != '',] # Eliminates those that are not noble
all.marriages <- marriages                                    # Keep marriages with intra-family ties
all.marriages.sn <- raph.data.frame(data.frame(all.marriages$husband_familyname_std,all.marriages$wife_familyname_std),directed=F)
...
<<r dup, echo=F, fig.cap="Graph representation of data processed using the \"duplication\" method, that duplicates the node for families that have intra-connections; \"original\" nodes are colored in blue, \"replicated\" nodes in gold.\\protect\\label{fig:dup}" >>=
