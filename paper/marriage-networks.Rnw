...
<<dup, echo=F, fig.cap="Graph representation of data processed using the \"duplication\" method, that duplicates the node for families that have intra-connections; \"original\" nodes are colored in blue, \"replicated\" nodes in gold.\\protect\\label{fig:dup}" >>=
