...
In this section, we will present several methods that would try to incorporate intra-family marriage data into the analysis of familiar networks. With every method, we will be interested in how the network measurements change, as well as how well it gives us insight into the analysis of the social network. What seems to be clear, in principle, is that any new method should consist in the creation of new nodes and edges linking these new, "artificial" nodes, to the old ones, since methods such as the pagerank and betweenness centrality use paths over edges for their computation; edges will have to correspond, somehow, to these intra links we want to incorporate into the graph modelling the social network. So any method proposed will mainly have to feature creation of new nodes.

The first method tested will be one which is relatively straightforward and simple: for every node with intra-node connections, add a new one that will be connected {\em only} to the family with a weight equivalent to the number of connections.
...
What we can see in Figure \ref{fig:sf} is that we can already obtain some insight on the network structure that might make splitting family nodes between husbands and wives meaningful. Some families seem to occupy the center through the "husband" nodes (blue), others through their "wife" nodes (gold). Another interesting thing is that a small sub-network that connects the females of a family to the males of others has been created; they can still be connected through their "other" parts, but they would need intra-family marriages for those parts to be connected to this small subnetwork. That is obviously an artifact of this representation; we cannot pretend that the female and male part of a family is disconnected even if there are not intra-marriages. However, taking this into account would lead to additional artifacts.
