...
The first method tested will be one which is relatively straightforward and simple: for every node with intra-node connections, add a new one that will be connected {\em only} to the it with a weight equivalent to the number of connections. Essentially, we convert a self-loop into an edge by creating a new node for the same family, with connections only to the original node.
