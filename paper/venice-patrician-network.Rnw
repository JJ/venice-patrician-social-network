...
The well known Contarini family is at the top of the ranking, with a total of
\Sexpr{100*length(marriages[ marriages$husband_familyname_std == 'Contarini' | marriages$wife_familyname_std == 'Contarini',]$year)/length(marriages$year)}\% of all weddings.
