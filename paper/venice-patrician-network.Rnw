...
for (r in V(venice.colleganza.sn)$name) {
  if (! is.null(family.type.dict[[r]])) {
    types <- c(types,"square")
    colors <- c(colors, family.colors[[family.type.dict[[r]]]])
  } else {
...
There are several conclusions that can be drawn from this. Most families, \Sexpr{ length(types[types == "square"])/length(types)} \% of them, who participated in these contracts would eventually become noble. Then, those with the best connections would be part of the initial batch of families that became nobles. 
\section{Conclusions}
