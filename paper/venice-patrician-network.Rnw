...
This data, which has been, as in the previous section, taken from \cite{10.1093/qje/qju006} is another interesting sample of the Venetian social network spanning from  \Sexpr{min(marriages[ !is.na(marriages$year),]$year)} to the demise of the Venetian republic in 1797, although the original data, extracted from the city archives, went beyond that date. The original data has been filtered by \begin{itemize}
